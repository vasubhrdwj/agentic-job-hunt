...
const OUTPUT = process.env.DEMO_CAPTURE_DIR
  ? path.resolve(process.env.DEMO_CAPTURE_DIR)
  : path.join(import.meta.dirname, "build", "capture");
const CHROME = "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome";

if (!EMAIL || !PASSWORD) {
  throw new Error("Set DEMO_EMAIL and DEMO_PASSWORD. Credentials are never written to disk.");
}
...
try {
  ...
  const amazonCard = page.locator("article").filter({ hasText: /Amazon/i }).first();
  await anchor(amazonCard, 92);
  ...
  const manifest = {
    ...
    viewport: { width: 1920, height: 1080 },
    note: "Credentials, cookies, and raw resume contents are not stored.",
  };
