...
1. Open `/login`. Confirm the private-access-key field has a visible label,
   focus is visible, Tab reaches the submit button, Enter submits, and an
   invalid key produces a readable error without entering the workspace.
